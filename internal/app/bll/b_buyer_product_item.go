...
// IBuyerProductItem 仕入商品詳細业务逻辑接口
type IBuyerProductItem interface {
	...
	// 创建数据
	Create(ctx context.Context, item schema.BuyerProductItem) (*schema.BuyerProductItem, error)
	// 更新数据
	Update(ctx context.Context, recordID string, item schema.BuyerProductItem) (*schema.BuyerProductItem, error)
	// 删除数据
